...
- Cavalry 내장 Lottie 익스포트 기능을 통한 직접 출력 파이프라인
- 해상도 독립적인 벡터 애니메이션 재생 (`lottie-react` / SVG 렌더러)
- `src/data/animations.js`에 항목을 추가하는 것만으로 아카이브를 확장할 수 있는 데이터 주도형 구조
- 현재 등록된 Lottie 파일은 초기 테스트 단계의 샘플이며, 이후 작업물이 완성될 때마다 지속적으로 추가됩니다
...
신문·학술 출판물 레이아웃을 모티프로 한 에디토리얼 디자인 스타일을 적용하고 있습니다.
